...
[简体中文](README.md) · [English documentation](docs/en/README.md) · [中文文档](docs/zh-CN/README.md) · [Release guide](docs/en/releasing.md)
...
Requires **macOS 13+**. On this repository's **Releases** page, choose `macos-arm64` for Apple Silicon or `macos-x86_64` for Intel. Extract the ZIP and move `墨笺.app` to Applications. Default CI builds are ad-hoc signed and are not Apple-notarized; see the [installation guide](docs/en/README.md) for first-launch details and building locally.
...
make package      # Versioned ZIP + SHA-256 for the current Mac architecture
make release      # Push a version tag; wait for GitHub CI to publish release assets
```

[CI](.github/workflows/ci.yml) checks Apple Silicon and Intel builds on branch pushes and pull requests, and retains packages, reports, and screenshots. The [Release workflow](.github/workflows/release.yml) publishes two ZIP files and `SHA256SUMS.txt` only after both platforms pass. Interrupted uploads leave an unpublished draft for recovery.
