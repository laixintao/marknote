...
`make test`: **13 tests, 388 assertions, no failures**.
...
`bash Scripts/smoke-test.sh release`: **65 checks passed** using a separate test application identifier.
...
The latest local native report and captures are in `.build/smoke-20260918-152725-h4EHhs/`. Synthetic editing dispatches an AppKit event before saving so document editing activities can finish even on an idle CI desktop. A watchdog captures process stacks if a native test times out.
